...
- deterministic `before_dispatch` interception with `before_agent_run` defense in depth: only an active Creator grant or a startup-validated non-blocked Business Admin reaches the model;
...
Username and display-name fields are not present in the confirmed command, tool-factory, `before_dispatch`, or `before_agent_run` identity context. The database supports bounded untrusted snapshots, and `AccessService` stores them when a trusted adapter can supply them, but they are never required and never participate in authorization.
